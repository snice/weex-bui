...
export default {
  ...
  computed: {
    ...
    cardWidth() {
      return (
        (this.cardLength - 1) * this.cardS.width + this.containerS.width + 235
      );
    }
  },
  created() {
  ...
  methods: {
    ...
    onPanMove(e) {
      ...
      const currentCardLeft =
        this.currentIndex * (this.cardS.width + this.cardS.spacing);
      const sliderCtn = this.$refs[`sliderCtn_${this.sliderId}`];
      sliderCtn &&
      ...
      if (this.cardS.scale !== 1) {
        const currentCard = this.$refs[
          `card${this.loopedIndex(index, cardLength)}_${this.sliderId}`
        ][0];
        currentCard &&
          animation.transition(
            currentCard,
            {
              styles: {
                transform: `scale(${1 -
                  (Math.abs(moveX) / this.cardS.width) *
                    (1 - this.cardS.scale)})`
              },
              timingFunction: "ease",
              delay: 0,
        ...
        // 左边的卡片
        const leftCard = this.$refs[
          `card${this.loopedIndex(index - 1, cardLength)}_${this.sliderId}`
        ][0];
        ...
        if (leftCard && index !== 0) {
          animation.transition(
            leftCard,
            {
              styles: {
                transform: `scale(${1 -
                  (Math.abs(moveX - this.cardS.width) / this.cardS.width) *
                    (1 - this.cardS.scale)})`
              },
              timingFunction: "ease",
              delay: 0,
        ...
        rightCard &&
          animation.transition(
            rightCard,
            {
              styles: {
                transform: `scale(${1 -
                  (Math.abs(this.cardS.width + moveX) / this.cardS.width) *
                    (1 - this.cardS.scale)})`
              },
              timingFunction: "ease",
              delay: 0,
    ...
    panEnd(e) {
      ...
      const isPullMore =
        selectIndex === this.cardLength - 1 &&
        (moveX < -68 || (moveX < -10 && duration < 200));
      if (isPullMore) {
    ...
    slideTo(originIndex, selectIndex) {
      ...
      const currentCard = this.$refs[
        `card${this.loopedIndex(originIndex, this.cardLength)}_${this.sliderId}`
      ][0];
      currentCard &&
        animation.transition(
          currentCard,
      ...
      const leftCard = this.$refs[
        `card${this.loopedIndex(originIndex - 1, this.cardLength)}_${
          this.sliderId
        }`
      ][0];
      ...
      sliderCtn &&
        animation.transition(
          ...
          {
            styles: {
              transform: `translateX(-${selectIndex *
                (this.cardS.width + this.cardS.spacing)}px)`
            },
            timingFunction: "ease",
            duration
          },
    ...
    bindExp(element) {
      if (element && element.ref) {
        ...
        if (this.cardS.scale !== 1) {
          const currentCardExp = `1-abs(x)/${this.cardS.width}*${1 -
            this.cardS.scale}`;
          const leftCardExp = `1-abs(x-${this.cardS.width})/${
            this.cardS.width
          }*${1 - this.cardS.scale}`;
          const rightCardExp = `1-abs(${this.cardS.width}+x)/${
            this.cardS.width
          }*${1 - this.cardS.scale}`;
          args.push({
          ...
          if (index === 0 && this.$refs[`card${index + 1}_${this.sliderId}`]) {
            ...
            index === this.cardLength - 1 &&
            this.$refs[`card${index - 1}_${this.sliderId}`]
          ) {
        ...
        const gesTokenObj = Binding.bind(
          ...
          e => {
            if (
              !this.isMoving &&
              (e.state === "end" || e.state === "cancel" || e.state === "exit")
            ) {
              this.panEnd(e);
            }
          }
    ...
    checkNeedAutoPlay() {
      if (this.autoPlay) {
        ...
        this.autoPlayTimer = setInterval(() => {
          this.slideTo(
            this.currentIndex,
            this.loopedIndex(this.currentIndex + 1, this.cardLength)
          );
    ...
    manualSetPage(selectIndex) {
      ...
      this.slideTo(
        this.loopedIndex(selectIndex - step, this.cardLength),
        selectIndex
      );
      setTimeout(() => {
        this.checkNeedAutoPlay();
