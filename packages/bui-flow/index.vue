<template>
    <div class="bui-flow-box">
        <div class="bui-flow" v-for="(item, index) in newItems" :key="index" @click="_click(index)">
    ...
    initItems(items) {
      ...
      return items.map((item, index) => {
        ...
        if (item.highlight) {
          ...
          if (highlightPointInnerColor)
            item.pointStyle.backgroundColor = highlightPointInnerColor;
          if (highlightPointBorderColor)
            item.pointStyle.borderColor = highlightPointBorderColor;
        }
        return item;
      });
    }
...
<style lang="scss" src="../../theme/css/flow.scss"/>