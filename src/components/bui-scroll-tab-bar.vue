<template>
	<scroller :show-scrollbar="false" scroll-direction="horizontal" class="tab-bar" :style="{height:(tabStyles().height)+'px',width:width+'px'}">
		<div class="tab-bar__item" :ref="'tabbar-item'+index" v-for="(item,index) in tabs" @click="handleClick($event,item,index)" :style="tabStyles(index)">
			<bui-image v-if="item.pic || item.selectedPic" class="tab-bar__image" :src="index === current ? item.selectedPic || item.pic : item.pic" :width="imageStyles().width" :height="imageStyles().height" :radius="imageStyles().radius" @click="handleClick($event,item,index)"></bui-image>
			...
			<text class="tab-bar__text" :style="{color : index == current ? tabStyle.selectedColor : tabStyle.color,fontSize:tabStyle.fontSize}">{{item.title}}</text>
		</div>
	</scroller>
</template>
<script>
